...
pub mod test {
    ...
    fn test_apply_fee_range_respects_session_max() {
        ...
        let higher_rate = Some(FeeRate::from_sat_per_vb_unchecked(10));
        let result = receiver.apply_fee_range(None, higher_rate);

        match result.0 {
            Ok(_) => (),
            Err(_) => panic!("Expected successful fee range application, but got error"),
        }
    }
    ...
    fn test_apply_fee_range_with_lower_max() {
        ...
        let result = receiver.apply_fee_range(None, lower_rate);
    ...
    fn test_apply_fee_range_with_none_uses_session_max() {
        ...
        let result = receiver.apply_fee_range(None, None);
